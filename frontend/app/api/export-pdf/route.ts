...
async function generateDefensePdf(payload?: any): Promise<Uint8Array> {
  ...
  page.drawText('VECTORNET COMPLIANCE AUDITOR // SIH 2026 PS 26155', {
    x: 140,
    y: height - 19,
    size: 9,
    font: timesBold,
  ...
  // Header Title
  page.drawText('VECTORNET SECURITY COMPLIANCE REPORT', {
    x: 40,
    y: height - 65,
    size: 16,
    font: timesBold,
  ...
  page.drawText('Multi-Vendor Automated Security Baseline Audit', {
    x: 40,
    y: height - 80,
    size: 10,
    font: timesRomanFont,
  ...
  page.drawText('COMPLIANCE STATUS', { x: 395, y: cardY + 34, size: 8, font: timesBold, color: rgb(0.1, 0.25, 0.6) });
  page.drawText(score >= 70 ? 'PASS' : 'NEEDS ATTENTION', { x: 395, y: cardY + 14, size: 8, font: timesBold, color: score >= 70 ? rgb(0.1, 0.5, 0.2) : rgb(0.8, 0.1, 0.1) });
  ...
  // Dynamic Remediation Section
  const remY = currentY - 35;
  page.drawText('PROPOSED CLI REMEDIATION COMMANDS', { x: 40, y: remY, size: 11, font: timesBold, color: rgb(0.1, 0.15, 0.25) });
  page.drawText(`Syntactically validated for ${vendor} with verification and rollback commands:`, { x: 40, y: remY - 14, size: 8, font: timesRomanFont, color: rgb(0.4, 0.45, 0.5) });
  ...
  if (failing.length === 0) {
    ...
    playbookLines.push('! No remediation or rollback commands required.');
  } else {
  ...
  page.drawText('Generated by VectorNet | Network Security Compliance Auditor', {
    x: 130,
    y: 10,
    size: 8,
    font: timesRomanFont,
...
export async function GET(request: NextRequest) {
      ...
      if (response.ok) {
        ...
        return new NextResponse(arrayBuffer, {
          ...
          headers: {
            ...
            'Content-Disposition': download ? 'attachment; filename="compliance_report.pdf"' : 'inline',
          },
        });
      }
    } catch {
  ...
  return new NextResponse(Buffer.from(pdfBytes), {
    ...
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': download ? 'attachment; filename="compliance_report.pdf"' : 'inline; filename="compliance_report.pdf"',
...
export async function POST(request: NextRequest) {
  try {
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      ...
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': 'inline; filename="compliance_report.pdf"',
      },
    });
  } catch (err: any) {
