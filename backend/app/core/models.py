"""
VectorNet Domain Models & Schemas
==================================
Canonical Security Baseline Model (SBM), OSCAL Line-Span Findings,
Unified Telemetry Schemas, and Vector Mapping Definitions.
Aligned with SIH Problem Statement 26155 (NTRO / NCIIPC).
"""
...
class DeviceType(str, Enum):
    """Supported network device classifications."""
    ROUTER = "router"
    SWITCH = "switch"
    FIREWALL = "firewall"
...
class PasswordHashing(str, Enum):
    """Cryptographic hashing standard classifications."""
    SHA256 = "sha256"
    MD5 = "md5"
    TYPE_7 = "type_7"
    PLAINTEXT = "plaintext"
...
class DeviceMetadata(BaseModel):
    """Device identification and operating system metadata."""
...
class AuthenticationSecurity(BaseModel):
    """Management-plane authentication and transport security posture."""
    ssh_version: int = 1
...
class AccessControl(BaseModel):
    """Access control list and brute-force mitigations."""
    management_acl_applied: bool = False
    login_block_failed_attempts: bool = False


# Extended / legacy models for multi-framework compliance views
...
class SecurityBaselineModel(BaseModel):
    """
    Canonical vendor-neutral Security Baseline Model (SBM).
    Normalizes multi-vendor CLI commands into a single structured schema
    paired with cryptographic line-span evidence.
    """
    ...
    # Optional multi-framework attributes
...
class DeviceAsset(BaseModel):
    """Discovered network infrastructure asset representation."""
    device_id: str
    hostname: str
    vendor: str
    model_number: str
...
class UnifiedJsonLog(BaseModel):
    """Normalized network syslog telemetry log."""
    timestamp: str
    device_id: str
    hostname: str
    vendor: str
...
class ComplianceStatus(str, Enum):
    """5-State audit compliance classification."""
    PASS = "PASS"
    FAIL = "FAIL"
    WARNING = "WARNING"
    UNKNOWN = "UNKNOWN"
...
class AuditFinding(BaseModel):
    """
    Individual audit finding control result with OSCAL line spans
    and proposed remediation syntax.
    """
    rule_id: str
    framework: str
    control_ref: str
...
class VectorMappingRequest(BaseModel):
    """Request payload for training novel vendor syntax into vector space."""
    raw_command: str
    mapped_category: str
    parsed_value: Any = 0
...
class VectorMappingResponse(BaseModel):
    """Response returned upon successful vector embedding registration."""
    status: str
    vector_id: str
    raw_command: str
    mapped_category: str
...
class AuditLogEntry(BaseModel):
    """Immutable RBAC audit trail record."""
    id: str
    user_id: str
    user_role: str
    action: str
...
class TrainRuleRequest(BaseModel):
    """Payload to write a learned syntax rule into a vendor skill markdown profile."""
    vendor: str
    cli_snippet: str
    target_sbm_key: str
