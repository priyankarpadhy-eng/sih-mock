...
===================================================
1. Injects global system prompt (global.md) specifying universal audit controls & output schemas.
2. Fingerprints vendor dialect using regex/NLP detection.
3. Dynamically injects vendor-specific skill file (syslog dialect, noisy pattern suppression, CLI semantics).
4. Coordinates multi-key OpenRouter AI execution with deterministic local SBM rule fallback.
"""
...
class AuditOrchestrator:
    ...
    def run_audit(self, raw_text: str, vendor: Optional[str] = None, user_query: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "deterministic_summary": deterministic_summary.model_dump() if hasattr(deterministic_summary, "model_dump") else deterministic_summary,
